...
def print_room(data):
	...
	#if room above exists and has no floor (room-data string has floor id in character [9] in bits [2:1] (mod 8) and 7 means "no floor")
	nextroom = next_room(data["RoomID"], [0, 0, -1])
	if (Room_list1.get(nextroom) is not None):
		if ((int(Room_list1[nextroom][9], 16) % 8) == 7):
			res += " UP: " + nextroom
	res += "\n"
	...
	if data["Door3"][0] == "W" and data["Door3"][1] == "":
		...
		res += nowall * ((longuest+2) * 8)
	if (floor == "."):
		res += " DW: " + next_room(data["RoomID"], [0, 0, 1])
	res += "\n"
